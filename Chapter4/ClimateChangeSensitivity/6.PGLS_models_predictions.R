## plotting PGLS predictions (sensitivity ~ traits)
...
Mammals <- readRDS("../Results/5.PGLS_models_results/Species_over_100km/Mammals_diet_v2.rds")
Reptiles <- readRDS("../Results/5.PGLS_models_results/Species_over_100km/Reptiles_diet_v2.rds")
Birds <- readRDS("../Results/5.PGLS_models_results/Species_over_100km/Birds_diet_v2.rds")
Amphibians <- readRDS("../Results/5.PGLS_models_results/Species_over_100km/Amphibians_diet_without_DB_v2.rds")
...
Predict_continuous <- function(Trait, Model) {
  ...
  Resample_predict <- function(Summary, Trait, Newdata){
    ...
    # predict
      if(pValsCoefs[1]<=0.05 & pValsCoefs[2]>0.05 & pValsCoefs[3]>0.05){
        Newdata$preds <- Intercept+Coefs[1]*Newdata$Var
        }
      if(pValsCoefs[1]<=0.05 & pValsCoefs[2]<=0.05 & pValsCoefs[3]>0.05){
        Newdata$preds <- Intercept+Coefs[1]*Newdata$Var+Coefs[2]*Newdata$Var^2
        }
      if(pValsCoefs[1]<=0.05 & pValsCoefs[2]<=0.05 & pValsCoefs[3]<=0.05){
        Newdata$preds <- Intercept+Coefs[1]*Newdata$Var+Coefs[2]*Newdata$Var^2+Coefs[3]*Newdata$Var^3
        }
      if(pValsCoefs[1]>0.05 & pValsCoefs[2]>0.05 & pValsCoefs[3]<=0.05){
        Newdata$preds <- Intercept+Coefs[1]*Newdata$Var+Coefs[2]*Newdata$Var^2+Coefs[3]*Newdata$Var^3
        }
  ...
  preds <- sapply(1:1000, FUN=function(i){
    Resample_predict(Summary, Trait, Newdata)
    }) %>% 
    as.data.frame()
  ...
  if(pValsCoefs[1]<=0.05 & pValsCoefs[2]<=0.05 & pValsCoefs[3]>0.05){
    preds$Median <- Intercept+Coefs[1]*preds$var+Coefs[2]*preds$var^2
  }
  if(pValsCoefs[1]<=0.05 & pValsCoefs[2]<=0.05 & pValsCoefs[3]<=0.05){
    preds$Median <- Intercept+Coefs[1]*preds$var+Coefs[2]*preds$var^2+Coefs[3]*preds$var^3
  }
  if(pValsCoefs[1]>0.05 & pValsCoefs[2]>0.05 & pValsCoefs[3]<=0.05){
    preds$Median <- Intercept+Coefs[1]*preds$var+Coefs[2]*preds$var^2+Coefs[3]*preds$var^3
  }
...
## plotting
to_plot <- rbind(Plot_continuous("log10_Body_mass_g", c("Amphibians", "Birds", "Mammals", "Reptiles"), list(Amphibians, Birds, Mammals, Reptiles)),
                 Plot_continuous("log10_Lifespan_proxy", c("Birds", "Reptiles"), list(Birds, Reptiles)),
                 Plot_continuous("log10_Litter_size", c("Amphibians","Birds", "Mammals"), list(Amphibians, Birds, Mammals, Reptiles)),
                 Plot_continuous("sqrt_Habitat_breadth_IUCN", c("Amphibians","Birds", "Mammals", "Reptiles"), list(Amphibians, Birds, Mammals, Reptiles)),
                 Plot_continuous("log10_Range_area", c("Amphibians","Birds", "Mammals", "Reptiles"), list(Amphibians, Birds, Mammals, Reptiles)),
...
to_plot$Trait <- factor(to_plot$Trait, levels=c( "log10_Range_area", "sqrt_Habitat_breadth_IUCN", "sqrt_Diet_breadth",
                                                 "log10_Body_mass_g", "log10_Lifespan_proxy", "log10_Litter_size"),
                        labels=c( "(a) Range area (km2, log10)", "(b) Habitat breadth (square-root)", "(c) Diet breath (square-root)",
                                  "(d) Body mass (g, log10)", "(e) Lifespan proxy (days, log10)", "(f) Litter/clutch size (log10)"))
...
print(p)
ggsave(p, filename="E:/Habitat_vars_predict_vertebrate_sensitivity_better_than_lht/Habitat_vars_predict_vertebrate_sensitivity_better_than_lht/Figures/PGLS_predictions_continuous.pdf",
       width = 8, height = 6)
...
Diet <- rbind(D1, D2, D3, D4)
Diet$Trait <- c("IN", "OM", "PL|SE", "VE", "FR|NE","IN", "OM", "PL|SE", "VE", "FR|NE", "IN", "OM", "PL|SE", "VE", "IN","OM", "VE")
Diet$facet <- "Diet"
#Diet$hline <- ifelse(grepl("Intercept", rownames(Diet)), Diet$Estimate, NA) 
Diet$Trait <- factor(Diet$Trait, levels=c( "FR|NE","PL|SE","IN", "VE","OM"), labels = c("Fruit/nectar", "Plant/seed", "Invertebrate", "Vertebrate","Omnivore"))

# X <- AmphibiansDiet$data$data
# X %>%  group_by(Primary_diet) %>%  summarise(C=n())

Diet$Estimate[Diet$Class=="Amphibians" & Diet$Trait=="Plant/seed"]  <- NA
Diet$Lower[Diet$Class=="Amphibians" & Diet$Trait=="Plant/seed"]  <- NA
Diet$Upper[Diet$Class=="Amphibians" & Diet$Trait=="Plant/seed"]  <- NA

Diet$Estimate[Diet$Class=="Amphibians" & Diet$Trait=="Vertebrate"]  <- NA
Diet$Lower[Diet$Class=="Amphibians" & Diet$Trait=="Vertebrate"]  <- NA
Diet$Upper[Diet$Class=="Amphibians" & Diet$Trait=="Vertebrate"]  <- NA
...
ggsave(PCat, filename="E:/Habitat_vars_predict_vertebrate_sensitivity_better_than_lht/Habitat_vars_predict_vertebrate_sensitivity_better_than_lht/Figures/PGLS_predictions_categories.pdf",
       width = 8, height = 6)
