...
Get_effects_cat <- function(Model, Trait, Class) {
  ...
  if(Class=="Reptiles"|((Trait=="VE"|Trait=="PL|SE") & Class=="Mammals")){
    Sub$Effect <- outer(c("PV", "SV", "PF", "AGR", "Light use", "Intense use"), paste0(":", Trait), FUN="paste0")
    Sub$Effect<- factor(Sub$Effect, levels=outer(c("PV", "SV", "PF", "AGR", "Light use", "Intense use"), paste0(":", Trait), FUN="paste0"))
  } 
  
  
  # if((Trait=="VE"|Trait=="PL|SE") & Class=="Mammals"){
  #   Sub$Effect <- outer(c("PV", "SV", "PF", "AGR", "Light use", "Intense use"), paste0(":", Trait), FUN="paste0")
  # Sub$Effect<- factor(Sub$Effect, levels=outer(c("PV", "SV", "PF", "AGR", "Light use", "Intense use"), paste0(":", Trait), FUN="paste0"))
  # }
...
ggplot(SpecialisationEffect,
       aes(Effect, Estimate, shape=Class, group=Class, col=Class, ymin = CI_low, ymax = CI_up)) +
...
Plot_DA <- 
ggplot(DaEffect,
       aes(Effect, Estimate, shape=Class, group=Class, ymin = CI_low, ymax = CI_up, col=Class)) +
  geom_rect(xmin=0, xmax=1.5,ymin=-Inf,ymax=Inf, fill="lightgrey", col=NA) +
  geom_rect(xmin=2.5, xmax=3.5,ymin=-Inf,ymax=Inf, fill="lightgrey", col=NA) +
  geom_rect(xmin=4.5, xmax=5.5,ymin=-Inf,ymax=Inf, fill="lightgrey", col=NA) +
  geom_rect(xmin=6.5, xmax=8,ymin=-Inf,ymax=Inf, fill="lightgrey", col=NA) +
...
MammalsPD$Effect <- c("PV:Diet", "SV:Diet", "PF:Diet", "AGR:Diet", "UR:Diet", "Light use:Diet", "Intense use:Diet",
                      "PV:Diet", "SV:Diet", "PF:Diet", "AGR:Diet", "Light use:Diet", "Intense use:Diet",
                      "PV:Diet", "SV:Diet", "PF:Diet", "AGR:Diet", "UR:Diet", "Light use:Diet", "Intense use:Diet",
                      "PV:Diet", "SV:Diet", "PF:Diet", "AGR:Diet", "Light use:Diet", "Intense use:Diet")
...
Plot_PD <- 
ggplot(PDeffects,
       aes(Effect, Estimate, shape=Diet, group=Diet, col=Diet, ymin = CI_low, ymax = CI_up)) +
  geom_rect(xmin=0, xmax=1.5,ymin=-Inf,ymax=Inf, fill="lightgrey", col=NA) +
  geom_rect(xmin=2.5, xmax=3.5,ymin=-Inf,ymax=Inf, fill="lightgrey", col=NA) +
  geom_rect(xmin=4.5, xmax=5.5,ymin=-Inf,ymax=Inf, fill="lightgrey", col=NA) +
  geom_rect(xmin=6.5, xmax=8,ymin=-Inf,ymax=Inf, fill="lightgrey", col=NA) +
  ...
  #viridis::scale_color_viridis(discrete = TRUE, option = "D",  end = 0.7) +
...
# Amphibians_diet <- readRDS("../Results/Single_predictor_models/Amphibians_diet.rds")
# Reptiles_diet <- readRDS("../Results/Single_predictor_models/Reptiles_diet.rds")
# 
# Amphibians_diet@frame$Primary_diet %>%  unique
# Reptiles_diet@frame$Primary_diet %>%  unique
# 
# Amph_OM<- Get_effects_cat(Amphibians_diet, "OM", "Amphibians")
# Amph_OM$Diet <- "OM"
# 
# Rep_OM<- Get_effects_cat(Reptiles_diet, "OM", "Reptiles")
# Rep_OM$Diet <- "OM"
# Rep_VE<- Get_effects_cat(Reptiles_diet, "VE", "Reptiles")
# Rep_VE$Diet <- "VE"
...
ggsave(Plot, filename="G:/Habitat_vars_predict_vertebrate_sensitivity_better_than_lht/Habitat_vars_predict_vertebrate_sensitivity_better_than_lht/Supporting Information/Figures/Full_LU_models_effects/Land_use_categorical_traits.pdf",
       width=12, height = 11)
