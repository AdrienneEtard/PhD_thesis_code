...
###########################################################################################################################################
Model <- readRDS("../Results/8.Occurrence_models/Model_occurrence_3_way_LU_UI_TL_Res_PD.rds")
...
preds_Slopes$Primary_diet <- factor(preds_Slopes$Primary_diet, labels = c("Fruit/nectar", "Invertebrate", "Omnivore", "Plant/seed", "Vertebrate"))
...
Slopes_plot <-
  ...
  geom_errorbar(width=.2, size=0.6, position=position_dodge(width = 0.7), stat="identity") + 
...
Slopes_plot
ggsave(Slopes_plot, filename="../Results/Figures/8.Occurrence_models/PD_Model_3_ways_Slopes.pdf", width=9, height = 4)

## checking which land-use intensities are signif different from minimal within each land uses
Signif_UI <- function(preds_Slopes, PD) {
  
  sub <- subset(preds_Slopes, Primary_diet==PD)
...
preds_SlopesFRNE <- Signif_UI(preds_Slopes, "Fruit/nectar")
preds_SlopesIN <- Signif_UI(preds_Slopes, "Invertebrate")
preds_SlopesOM <- Signif_UI(preds_Slopes, "Omnivore")
preds_SlopesPLSE <- Signif_UI(preds_Slopes, "Plant/seed")
preds_SlopesVE <- Signif_UI(preds_Slopes, "Vertebrate")

Signif <- rbind(preds_SlopesFRNE, preds_SlopesIN, preds_SlopesOM, preds_SlopesPLSE, preds_SlopesVE) %>%  
  filter(Significance==TRUE)

## checking which relationships are similar for minimal uses
preds2 <-  data.frame(Median=apply(X=preds, MARGIN=1, FUN=median),
                      Upper=apply(X=preds, MARGIN=1, FUN=quantile, probs=0.975),
                      Lower=apply(X=preds, MARGIN=1, FUN=quantile, probs=0.025))
preds2 <- cbind(preds2, Newdata)
preds2 %>%  
  filter(Use_intensity=="Minimal use", LandUse=="Primary vegetation") %>% 
...
## significant cases for minimal intensity
# primary vegetation, plantation, cropland (FR|NE)
# pasture, urban (IN)
# pasture, urban (OM)
# cropland, urban (PL|SE)
# plantation, pasture, cropland (VE)

Predictions$Primary_diet <- factor(Predictions$Primary_diet,
                                   labels = c("Fruit/nectar", "Invertebrate", "Omnivore", "Plant/seed", "Vertebrate"))

Predictions_minimal <- subset(Predictions, Use_intensity=="Minimal use")
Predictions_minimal$Significant <- FALSE
Predictions_minimal$Significant[Predictions_minimal$LandUse %in% c("Primary", "Plantation", "Cropland") &
                                  Predictions_minimal$Primary_diet=="Fruit/nectar"] <- TRUE
Predictions_minimal$Significant[Predictions_minimal$LandUse %in% c("Pasture", "Urban") &
                                  Predictions_minimal$Primary_diet=="Invertebrate"] <- TRUE
Predictions_minimal$Significant[Predictions_minimal$LandUse %in% c("Pasture", "Urban") &
                                  Predictions_minimal$Primary_diet=="Omnivore"] <- TRUE
Predictions_minimal$Significant[Predictions_minimal$LandUse %in% c("Cropland", "Urban") &
                                  Predictions_minimal$Primary_diet=="Plant/seed"] <- TRUE
Predictions_minimal$Significant[Predictions_minimal$LandUse %in% c("Plantation", "Pasture","Cropland") &
                                  Predictions_minimal$Primary_diet=="Vertebrate"] <- TRUE

p_continuous_effects <- 
ggplot(Predictions_minimal, aes(Residual_BMR_log_log, Estimate, ymin=Lower, ymax=Upper))+
  geom_line(aes(linetype=Significant), size=0.8) +
  scale_linetype_manual(values=c("dashed", "solid"))+
  geom_ribbon(alpha=0.05, col=NA) + 
  facet_grid(LandUse~Primary_diet)+
...
p_manuscript <- 
  ggarrange(  Slopes_plot +
              ...
              p_continuous_effects +
              ggtitle("(b) Predicted effects of residual RMR on occurrence probability (minimal use intensity)"),
            nrow=2,
            heights = c(0.40, 0.60))
p_manuscript

ggsave(p_manuscript, 
       filename = "../Results/Figures/8.Occurrence_models/Figure_predictions_slopes_PD.pdf", height=11,width=9)

ggsave(p_manuscript, 
       filename = "../Results/Figures/8.Occurrence_models/Figure_predictions_slopes_PD.png", height=11, width=9)
